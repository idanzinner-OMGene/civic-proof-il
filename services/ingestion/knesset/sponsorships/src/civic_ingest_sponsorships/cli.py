"""CLI: ``python -m civic_ingest_sponsorships run``."""
...
MANIFEST_PATH = REPO_ROOT / "services/ingestion/knesset/manifests/sponsorships.yaml"


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="civic_ingest_sponsorships")
    ...
    from .normalize import normalize_bill
    ...
    with IngestRun(source_family=manifest.family) as run:
        ...
        upsert_fn = (lambda _n: None) if args.dry_run else upsert_bill
        result = run_adapter(
            ingest_run=run,
            source_url=str(manifest.source_url),
            fetch=fetcher.fetch,
    ...
    print(
        f"sponsorships: pages={result.pages} rows={result.rows_parsed} "
