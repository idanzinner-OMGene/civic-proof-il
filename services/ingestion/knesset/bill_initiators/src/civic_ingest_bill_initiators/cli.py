"""CLI: ``python -m civic_ingest_bill_initiators run``."""
...
MANIFEST_PATH = (
    REPO_ROOT / "services/ingestion/knesset/manifests/bill_initiators.yaml"
)


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="civic_ingest_bill_initiators")
    ...
    from .normalize import normalize_bill_initiator
    ...
    with IngestRun(source_family=manifest.family) as run:
        ...
        upsert_fn = (
            (lambda _n: None) if args.dry_run else upsert_bill_sponsorship
        )
        result = run_adapter(
            ingest_run=run,
            ...
            fetch=fetcher.fetch,
            archive=_archive,
            parse=parse_bill_initiators,
            normalize=normalize_bill_initiator,
            upsert=upsert_fn,
            max_pages=args.max_pages,
        )

    print(
        f"bill_initiators: pages={result.pages} rows={result.rows_parsed} "
