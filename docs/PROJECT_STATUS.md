...
- **Tests:** Workspace run (no Docker smoke): **695 passed, 0 skipped** (last run, 2026-05-04). **254/254** alignment smoke rows; **5/5** regression. **`make eval`** exits 0 with current gates.
...
| PR-6 | **Done** | Government decision ingestion (BudgetKey adapter + full claim pipeline) |
...
- **2026-05-04 (session 11 — V2 PR-6):** **V2 PR-6 delivered (Government Decision Ingestion + Claim Pipeline).** Recorded Tier-1 BudgetKey cassette (50 numbered government decisions, SHA-256 `e7c67cc6...`) with `SOURCE.md`. Built `civic-ingest-gov-decisions` adapter package under `services/ingestion/gov_il/`: `types.py` (`BudgetKeyDecisionRow`, `NormalizedGovernmentDecision`), `parse.py` (filters to formal "החלטות" policy types), `normalize.py` (deterministic `uuid5(NS, "budgetkey_gov_decision:{id}")`, government number extraction via `ה-\s*(\d+)`, 2000-char summary truncation), `upsert.py` (calls existing `government_decision_upsert.cypher`), `cli.py` (paginated BudgetKey fetch, `--max-pages`, archival). Added `"government_decisions"` to `AdapterKind` + `source_manifest.schema.json` enum. Extended claim pipeline end-to-end: `"government_decision"` ClaimType + `government_decision_id` slot added to `AtomicClaim` model + JSON schema; `SLOT_TEMPLATES["government_decision"]` added with required=`{government_decision_id}`, optional=`{speaker_person_id, target_person_id, office_id}`; 3 new regex decomposition rules (Hebrew `_HE_GOV_DECISION_NUMBER` / `_HE_GOV_DECISION_REF`, English `_EN_GOV_DECISION_NUMBER`); decomposer wired to map `decision_number` → `government_decision_id` slot; `infra/neo4j/retrieval/government_decision.cypher` created; `_government_decision()` handler added to verdict engine + `VerdictInputs.government_decision_id` field; pipeline `_SLOT_TO_KIND` + `_fallback_resolve` extended for `government_decision` entity kind. Neo4j relationship templates: `concerns_office.cypher`, `concerns_committee.cypher`, `concerns_party.cypher`, `refers_to_gov_decision.cypher`. Registered in `scripts/ingest_all.sh` (position 4, after elections) + root `pyproject.toml`. Added 28 tests (17 adapter unit + 11 rule). Alignment smoke updated: 16 new assertions, `SUPPORTED_CLAIM_TYPES` extended, relationship count 21→25. Full workspace: **690 passed, 0 failed**. Schema drift check: OK. **All V2 PRs complete.**
