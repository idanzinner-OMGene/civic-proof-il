...
# 4. gov_decisions     → GovernmentDecision nodes (BudgetKey API; no entity deps)
# 5. committees        → creates Committee nodes (Knesset 25)
# 6. committee_memberships → MEMBER_OF_COMMITTEE edges (requires Person + Committee)
# 7. sponsorships      → creates Bill nodes (Knesset 25)
# 8. bill_initiators   → SPONSORED edges (requires Person + Bill)
# 9. votes             → VoteEvent nodes + CAST_VOTE edges
# 10. attendance       → AttendanceEvent nodes + ATTENDED edges
# ---------------------------------------------------------------------------
...
run_adapter "gov_decisions"        "civic_ingest_gov_decisions"        "civic-ingest-gov-decisions"
